...
export default function AdminMembershipsPage() {
  ...
  }, [isAuthorizedAdmin, fetchApplications]);
  ...
  const handleApprove = async (app: MembershipApp) => {
    ...
    try {
      ...
      if (data?.success) {
        showToast(`Approved ${app.name} (${app.randomize_id})`);
        setApps(prev => prev.map(item => item.id === app.id ? { ...item, payment_status: 'approved' } : item));
      }
    } catch (err: any) {
  ...
  const handleReject = async (app: MembershipApp) => {
    if (!confirm(`Are you sure you want to reject ${app.name}?`)) return;
    setProcessingId(app.id);
    ...
    if (!error) {
      showToast(`Rejected application for ${app.name}`);
      setApps(prev => prev.map(item => item.id === app.id ? { ...item, payment_status: 'rejected' } : item));
    }
    setProcessingId(null);
  };
  ...
  const filteredApps = useMemo(() => {
    return apps.filter(a => {
      ...
      if (statusFilter === 'pending') return a.payment_status === 'pending_verification';
      if (statusFilter === 'approved') return a.payment_status === 'approved';
      return true;
    });
    ...
    return (
      <main className="min-h-screen bg-[#07050e] text-white flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-zinc-300">Verifying Admin Permissions...</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#07050e] text-white p-4 sm:p-8">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-[#0d0918] border border-white/10 p-3.5 rounded-2xl">
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Search Name, Reg No, UTR / Ref ID..."
              value={searchTerm}
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {['pending', 'approved', 'all'].map((mode) => (
              ...
              >
                {mode} ({apps.filter(a => mode === 'all' ? true : mode === 'pending' ? a.payment_status === 'pending_verification' : a.payment_status === 'approved').length})
              </button>
            ))}
          </div>
        </div>
                    ...
                    <tr key={a.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                          a.payment_status === 'approved' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
                          a.payment_status === 'rejected' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                          'bg-amber-500/20 text-amber-300 border border-amber-500/30'
                        }`}>
                          {a.payment_status === 'pending_verification' ? 'Pending' : a.payment_status}
                        </span>
                      </td>
      ...
      {/* Explicit Fixed-Width Preview Modal */}
      {activeImageModal && (
        <div 
              ...
              >
                ...
                {activeImageModal.startsWith('http') && (
                  <a
                    ...
                    className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-xs font-bold text-white transition-all inline-flex items-center gap-2"
                  >
