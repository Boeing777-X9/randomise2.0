...
export default function NewsletterComingSoonPage() {
    ...
    const handleSubmit = useCallback(
        async (e) => {
            ...
            // Simulating success state while Supabase is commented out
            setStatus("success");
            setMessage("You're on the list!");
        },
    ...
    return (
        <main className="w-full min-h-screen bg-[#020108] text-white font-sans relative overflow-x-hidden flex flex-col items-center justify-center px-4 sm:px-6">
            <div
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 opacity-30 z-0"
                style={{
                    background:
                        "radial-gradient(700px circle at 15% 10%, rgba(45,15,247,0.25), transparent 60%), radial-gradient(700px circle at 85% 90%, rgba(242,0,89,0.2), transparent 60%)",
                }}
            />
...
<div className="w-full max-w-4xl mx-auto rounded-2xl border border-purple-500/20 p-6 sm:p-10 shadow-xl backdrop-blur-md text-left bg-opacity-40 bg-[#0a051a]">                        <form onSubmit={handleSubmit} noValidate className="w-full">
                            <label
